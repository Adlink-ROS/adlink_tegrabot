...
#include <adlink_tegrabot/swarm_layer.h>
#include <math.h>
...
namespace neuronbot_swarm_layer
{
    void SwarmLayer::onInitialize()
    {
        ...
        // Dynamic recofigure
        server_ = new dynamic_reconfigure::Server<adlink_tegrabot::SwarmLayerConfig>(nh);
        f_ = boost::bind(&SwarmLayer::configure, this, _1, _2);
        server_->setCallback(f_);
    }
    
    void SwarmLayer::robotsCallback(const adlink_tegrabot::MultiRobots& robots) 
    {
    ...
    {
        std::list<adlink_tegrabot::Robot>::iterator r_it;
        
        for(r_it = transformed_robots_.begin(); r_it != transformed_robots_.end(); ++r_it)
        {
            adlink_tegrabot::Robot robot = *r_it;
    ...
    void SwarmLayer::updateBounds(double origin_x, double origin_y, double origin_z, double* min_x, double* min_y, double* max_x, double* max_y)
    {
        ...
        {
            adlink_tegrabot::Robot& robot = robots_list_.robots[i];
            adlink_tegrabot::Robot robot_tmp = robot; // copy of the original one
    ...
    void SwarmLayer::configure(adlink_tegrabot::SwarmLayerConfig &config, uint32_t level)
    {
